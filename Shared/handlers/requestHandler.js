...
async function NormalReq(path) {
  return await fetch(`${ path }`, { agent, headers: { "x-api-key": Config.braytechKey } }).then(async (request) => {
    try {
      const response = await request.text();
      if(Misc.IsJSON(response)) {
        const res = JSON.parse(response);
...
const SearchPrefixDestinyPlayer = async (username, callback) => { const { isError, Data } = await BungieReq(`/Platform/User/Search/Prefix/${username}/0/`); callback(isError, Data); }
const GetDailyMods = async (callback) => { const { isError, Data } = await NormalReq(`https://b.vlsp.network/vendor/?hash=350061650`); callback(isError, Data); }
const GetVendor = async (hash, callback) => { const { isError, Data } = await NormalReq(`https://b.vlsp.network/vendor/?hash=${hash}`); callback(isError, Data); }
...
module.exports = {
  ...
  GetGlobalHighestPowerMinusArtifactLeaderboard, GetGlobalDawning2020Leaderboard, GetDailyMods, GetVendor, GetCookies
}